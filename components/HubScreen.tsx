...
const HubScreen: React.FC<HubScreenProps> = ({ currentView, onChangeView }) => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 bg-stone-950 animate-fade-in">
      <div className="text-center mb-12">
        <h2 className="text-4xl md:text-5xl font-serif text-stone-200 tracking-widest uppercase mb-2">Nexus</h2>
        <div className="h-px w-24 bg-amber-900/50 mx-auto"></div>
        <p className="text-stone-600 text-xs uppercase tracking-[0.2em] mt-2">Hub de Criação</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl w-full">
        {menuItems.map((item) => (
          <button
            ...
            className={`group flex items-start gap-6 p-6 border border-stone-800 rounded-sm transition-all duration-300 text-left ${item.bg}`}
          >
            <div className={`mt-1 transition-colors ${item.color} group-hover:text-amber-500`}>
              {item.icon}
            </div>
            <div>
              <h3 className="text-xl font-serif text-stone-300 group-hover:text-amber-100 transition-colors mb-1">
                {item.label}
              </h3>
              <p className="text-sm text-stone-600 group-hover:text-stone-400 font-light leading-relaxed">
                {item.desc}
              </p>
            </div>
          </button>
